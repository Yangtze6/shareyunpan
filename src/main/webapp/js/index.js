    ...
    if(reg.trim() == "" || reg.trim() == null){
        ...
        $.post("file/searchFile.action", {
            ...
            if (data.success) {
                ...
                $.each(data.data, function() {
                    $("#list").append('<tr><td><input onclick="selectCheckbox()" name="check_name" type="checkbox" aria-label="..."></td>' +
                    '<td width="60%"><a href="#" prePath="' + this.currentPath +'" fileType="' + this.fileType +'" onclick="return openFile(this)">'
                          + this.fileName + '</a></td>' +'<td width="32px"> </td>' +'<td width="32px"><a href="#" class="glyphicon glyphicon-download-alt"
                          title="下载" onclick="return downloadFile(this)"></a></td>' + '<td width="32px"><a href="#"'
                          + 'class="glyphicon glyphicon-option-horizontal" title="更多"></a></td>' +
                    '<td>' + this.fileSize + '</td>' +
                    '<td>' + this.lastTime + '</td></tr>');
                });
